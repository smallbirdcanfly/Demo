package com.fz.cdh.pcdd.weight.timeselector;
...
import com.fz.cdh.pcdd.weight.timeselector.Utils.DateUtil;
import com.fz.cdh.pcdd.weight.timeselector.Utils.TextUtil;
import com.fz.cdh.pcdd.weight.timeselector.view.PickerView;
 ...
 * Created by xiaoge
 */
public class TimeSelector implements DialogInterface.OnDismissListener {
    ...
    private boolean canAfterToday;//是否可以晚于当前
    private TextView textView;


    public TimeSelector(Context context, TextView tv,ResultHandler resultHandler, String startDate, String endDate) {
    ...
    public void canAfterToday(boolean canAfterToday){
        this.canAfterToday = canAfterToday;
    }
    ...
    private void initView() {
        ...
        ll_top= (LinearLayout) seletorDialog.findViewById(R.id.ll_top);
        ...
        ll_top.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(!canAfterToday && selectedCalender.getTime().after(currentCalendar.getTime())){
                    T.showShort("不能晚于当前时间");
    ...
    private void initTimer() {
        ...
        if (spanYear) {
            ...
            if ((scrollUnits & SCROLLTYPE.MINUTE.value) != SCROLLTYPE.MINUTE.value) {
                second.add(fomatTimeUnit(startMininute));
            } else {
                for (int i = startMininute; i <= MAXMINUTE; i++) {
...
//                minuteChange();
    ...
    private void loadComponent() {
        ...
        for (int i = 0; i < year.size(); i++) {
            if (year.get(i).equals(String.valueOf(currentYear))) {
                year_pv.setSelected(i);
            }
        }
        for (int i = 0; i < month.size(); i++) {
            if (month.get(i).equals(String.valueOf(currentMonth).length() == 1 ? "0" + String.valueOf(currentMonth) : String.valueOf(currentMonth))) {
                month_pv.setSelected(i);
            }
        }
        for (int i = 0; i < day.size(); i++) {
            if (day.get(i).equals(String.valueOf(currentDay).length() == 1 ? "0" + String.valueOf(currentDay) : String.valueOf(currentDay))) {
                day_pv.setSelected(i);
            }
        }
        for (int i = 0; i < hour.size(); i++) {
            if (hour.get(i).equals(String.valueOf(currentHour).length() == 1 ? "0" + String.valueOf(currentHour) : String.valueOf(currentHour))) {
                hour_pv.setSelected(i);
            }
        }
        for (int i = 0; i < minute.size(); i++) {
            if (minute.get(i).equals(String.valueOf(currentMinute).length() == 1 ? "0" + String.valueOf(currentMinute) : String.valueOf(currentMinute))) {
                minute_pv.setSelected(i);
            }
        }
        for (int i = 0; i < second.size(); i++) {
            if (second.get(i).equals(String.valueOf(currentSecond).length() == 1 ? "0" + String.valueOf(currentSecond) : String.valueOf(currentSecond))) {
    ...
    private void excuteScroll() {
        ...
        second_pv.setCanScroll(second.size()>1&& (scrollUnits & SCROLLTYPE.MINUTE.value) == SCROLLTYPE.MINUTE.value);
    }
